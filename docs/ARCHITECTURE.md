...
| 2026-09-06 | Fishing animation: **leave vanilla's `FishingManager` alone** — it drives the cast/idle pose for free while a rod is held facing water (which `ISPZRPGFishAction` sets up). FISH-4 tried to drive `FishingStage` ourselves + suppress that manager; in-game it killed the animation (character stood still), so it was reverted. | Empirically the "do nothing, vanilla animates it" path works and the "take control" path didn't. `setActionAnim("Loot")` is the base anim under vanilla's fishing layer. |
